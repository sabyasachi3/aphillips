...
import com.xebialabs.aphillips.usertype.examples.take5.EntityWithStringBuilderProperty;

/**
 * Integration tests for the {@link DirtyCheckingStringBuilderUserType}.
 * 
 * @author anphilli
 * @since Nov 2, 2009
 */
@ContextConfiguration(locations = { "classpath:take5/dataSourceContext.xml "})
public class DirtyCheckingStringBuilderUserTypeItest extends AbstractTransactionalDatasourceItest {
    private final String propertyValue = "James";
    private Integer id;
    
    @Before
    public void populateDb() {
        EntityWithStringBuilderProperty holder = new EntityWithStringBuilderProperty(propertyValue);
        session.save(holder);
        session.flush();
    ...
    public void hydrate() {
        ...
        DirtyCheckingStringBuilder builder = holder.getBuilder();
    ...
    public void dehydrateReplaced() {
        ...
        DirtyCheckingStringBuilder newBuilder = new DirtyCheckingStringBuilder("James Bond");
        
        holder.setBuilder(newBuilder);
        session.flush();
        session.evict(holder);
        
        DirtyCheckingStringBuilder persistedBuilder = 
